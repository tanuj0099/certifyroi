...
// ─────────────────────────────────────────────────────────
// COMPASS — corner motif, top-right, small and quiet
// ─────────────────────────────────────────────────────────
      ...
      {Array.from({ length: 36 }).map(function(_, i) {
        ...
        return (
          <line key={i}
            ...
            stroke={isCard ? strokeO : strokeI}
            strokeWidth={isCard ? 0.8 : 0.45}
          />
        )
      })}
      {/* Cardinal letters */}
...
// ─────────────────────────────────────────────────────────
// ELEVATION MARK — minimal brand symbol
// ─────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────
// TRUST STRIP — bigger, slower, more premium
// ─────────────────────────────────────────────────────────
function TrustStrip() {
  var items = [
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <span style={{ fontFamily:FM, fontSize:'9px', color:C.text4, letterSpacing:'0.12em', textTransform:'uppercase' }}>
          Market data · Q1 2026
        </span>
      </div>
          ...
          {[...items,...items].map(function(item, i) {
            return (
              <div key={i} style={{
                ...
                <span style={{
                  fontFamily:FM, fontSize:'11px', color:C.text2,
                  letterSpacing:'0.015em', lineHeight:'1.4',
                }}>
                  <span style={{
                    fontFamily:FM, fontSize:'9px', color:C.green,
                    letterSpacing:'0.08em', textTransform:'uppercase',
                    marginRight:'10px', opacity:0.65,
                  }}>
                    {item.tag}
                  </span>
                  {item.text}
...
// ─────────────────────────────────────────────────────────
// CERT ASSEMBLY — sticky scroll, light mode
// ─────────────────────────────────────────────────────────
    ...
    <div ref={trackRef} style={{ height:'300vh', position:'relative' }}>
      <div style={{ position:'sticky', top:0, height:'100vh', width:'100%', display:'flex', alignItems:'center', justifyContent:'center' }}>
            ...
            <div style={{
              ...
              <div style={{ position:'absolute', inset:0, transform:l1 }}>
                <svg viewBox="0 0 480 340" width="100%" height="100%" style={{ position:'absolute', inset:0, display:'block' }}>
                  <defs>
                    <linearGradient id="certBord2" x1="0" y1="0" x2="1" y2="1">
                      <stop offset="0%" stopColor="#2D6A4F"/>
                      <stop offset="50%" stopColor="#4A8C6A"/>
                      <stop offset="100%" stopColor="#9A7235"/>
                    </linearGradient>
                  </defs>
                  <rect x="0" y="0" width="480" height="340" rx="4" fill="#FEFEFE" fillOpacity="0.98"/>
                  <rect x="1" y="1" width="478" height="338" rx="3.5" fill="none" stroke="url(#certBord2)" strokeWidth="1.2"/>
                  <rect x="8" y="8" width="464" height="324" rx="2" fill="none" stroke="rgba(26,25,22,0.05)" strokeWidth="0.6"/>
              ...
              <div style={{ position:'absolute', inset:0, transform:l2, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'clamp(16px,4vw,36px)' }}>
                <div style={{ fontFamily:FM, fontSize:'8px', color:'rgba(45,106,79,0.55)', letterSpacing:'0.28em', marginBottom:'11px', textTransform:'uppercase' }}>CERTIFYROI · INDIA 2026</div>
                <div style={{ fontFamily:FSER, fontWeight:'500', fontStyle:'italic', fontSize:'clamp(0.95rem,3.2vw,1.75rem)', letterSpacing:'-0.01em', color:C.text, marginBottom:'5px', textAlign:'center', lineHeight:1.1 }}>ROI Report</div>
                <div style={{ fontFamily:FB, fontSize:'clamp(10px,1.4vw,12px)', color:C.text3, marginBottom:'22px', textAlign:'center' }}>Personalised analysis · Your city · Your salary</div>
                <div style={{ display:'flex', gap:'clamp(10px,4vw,40px)', marginBottom:'18px' }}>
                  {[{label:'PAYBACK',value:'9 mo',color:C.green},{label:'5-YR GAIN',value:'₹14.2L',color:C.gold},{label:'DELTA',value:'+35%',color:C.greenL}].map(function(s,i){
                ...
                <div style={{ fontFamily:FM, fontSize:'7px', color:C.text4, letterSpacing:'0.12em', textAlign:'center' }}>VERIFIED · DATA: NAUKRI MARCH 2026</div>
              </div>
            </div>
          </div>
          <div style={{ opacity:hintOp, marginTop:'40px', textAlign:'center', pointerEvents:'none', transition:'opacity 0.3s' }}>
            <motion.div animate={{ y:[0,7,0] }} transition={{ duration:1.8, repeat:Infinity, ease:'easeInOut' }}>
              <div style={{ fontFamily:FM, fontSize:'10px', color:C.text3, letterSpacing:'0.2em', textTransform:'uppercase' }}>↓  scroll to build your report  ↓</div>
            </motion.div>
          </div>
        </div>
        <div style={{ opacity:assembledOp, position:'absolute', bottom:'8%', left:0, right:0, textAlign:'center', pointerEvents:'none', zIndex:5, transition:'opacity 0.3s' }}>
          <div style={{ fontFamily:FM, fontSize:'11px', color:C.green, letterSpacing:'0.2em', textTransform:'uppercase' }}>✓  ROI REPORT · READY</div>
        </div>
      </div>
    </div>
  )
      ...
      <div style={{ maxWidth:'1120px', margin:'0 auto', padding:isMobile?'0 24px':'0 80px' }}>
        ...
        <motion.div variants={RISE} initial="hidden" whileInView="show" viewport={{ once:true }} transition={{ delay:0.2 }}
          ...
          <div style={{ flex:1, minWidth:'200px' }}>
            ...
            <div style={{ fontFamily:FB, fontSize:'15px', color:C.text2, lineHeight:'1.8' }}>Across 17 domains — cloud, data, finance, medical, law, government. Every certification mapped to real hiring data from Naukri, AmbitionBox, and LinkedIn India.</div>
          </div>
        </motion.div>
      </div>
    </div>
...
function HowItWorks({ isMobile, onEnter }) {
  var steps = [
    { label:'Enter your details', subtitle:'Cert · salary · city', desc:'Search from 103 indexed certifications. Enter your current salary and role. Upload your resume for AI to read your actual background.', detail:'No account required.' },
    { label:'We run the numbers', subtitle:'Cost · payback · uplift', desc:'Total investment, expected salary increase, demand index, and exact payback month — calculated against your baseline, not a national average.', detail:'India market data · 2026' },
            ...
            <div style={{ position:'absolute', top:'9px', left:'9px', right:'9px', height:'1px', pointerEvents:'none', zIndex:0 }}>
              <svg width="100%" height="2" style={{ overflow:'visible', display:'block' }}>
                <motion.line x1="0" y1="1" x2="100%" y2="1"
                  ...
                  animate={routeInView?{pathLength:1,opacity:0.35}:{}}
            ...
            {steps.map(function(step, i) {
              ...
              return (
                <motion.div key={step.label} variants={RISE} initial="hidden" whileInView="show" viewport={{ once:true }} transition={{ delay:i*0.1 }}
                  ...
                  <div style={{ fontFamily:FH, fontWeight:'600', fontSize:'clamp(1rem,2.2vw,1.15rem)', color:C.text, letterSpacing:'-0.02em', marginBottom:'5px' }}>{step.label}</div>
                  ...
                  <div style={{ fontFamily:FB, fontSize:'14px', color:C.text2, lineHeight:'1.8', marginBottom:'12px' }}>{step.desc}</div>
            ...
            return (
              <motion.div key={i} variants={RISE} initial="hidden" whileInView="show" viewport={{ once:true }} transition={{ delay:i*0.07 }}>
                <div style={{ position:'relative', display:'inline-block', marginBottom:'14px', maxWidth:'100%' }}>
                  <div style={{ fontFamily:FH, fontWeight:'500', fontSize:'clamp(0.9rem,2vw,1.15rem)', color:C.text, opacity:0.18, letterSpacing:'-0.01em', lineHeight:1.4, wordBreak:'break-word' }}>{pair.wrong}</div>
                ...
                <div style={{ display:'flex', gap:'12px', alignItems:'flex-start' }}>
                  ...
                  <div style={{ fontFamily:FB, fontSize:'clamp(14px,1.8vw,15px)', color:C.text, lineHeight:'1.75', fontWeight:'400' }}>{pair.right}</div>
                </div>
              </motion.div>
            )
          })}
          ...
          {stories.map(function(s, i) {
            ...
            return (
              <motion.div key={i} variants={RISE} initial="hidden" whileInView="show" viewport={{ once:true }} transition={{ delay:i*0.1 }}
                ...
                <div style={{ fontFamily:FB, fontWeight:'500', fontSize:'14px', color:C.text, lineHeight:'1.65' }}>{s.answer}</div>
              </motion.div>
            )
          })}
        </div>
          ...
          {modes.map(function(m, i) {
            ...
            return (
              <motion.div key={m.label} variants={RISE} initial="hidden" whileInView="show" viewport={{ once:true }} transition={{ delay:i*0.1 }}
                ...
                <div style={{ fontFamily:FSER, fontStyle:'italic', fontSize:'17px', color:C.text3, marginBottom:'5px', fontWeight:'400' }}>{m.serif}</div>
                <div style={{ fontFamily:FH, fontWeight:'600', fontSize:'clamp(1rem,2vw,1.15rem)', color:C.text, letterSpacing:'-0.02em', marginBottom:'5px' }}>{m.label}</div>
                ...
                <div style={{ fontFamily:FB, fontSize:'14px', color:C.text2, lineHeight:'1.8' }}>{m.desc}</div>
              </motion.div>
            )
          })}
        </div>
  ...
  return (
    <div style={{ background:C.surface, borderTop:'1px solid '+C.border, borderBottom:'1px solid '+C.border, paddingTop:isMobile?'72px':'96px', paddingBottom:isMobile?'72px':'96px' }}>
      <div style={{ maxWidth:'1120px', margin:'0 auto', padding:isMobile?'0 24px':'0 80px' }}>
        ...
        <motion.h2 variants={RISE} initial="hidden" whileInView="show" viewport={{ once:true }}
          style={{ fontFamily:FH, fontWeight:'600', fontSize:'clamp(1.7rem,4vw,2.6rem)', color:C.text, letterSpacing:'-0.025em', lineHeight:1.1, marginTop:0, marginBottom:'64px' }}>
          They chose the right path.<br /><span style={{ color:C.green }}>It worked.</span>
        </motion.h2>
          ...
          {quotes.map(function(q, i) {
            ...
            return (
              <motion.div key={i} variants={SLIDE_L} initial="hidden" whileInView="show" viewport={{ once:true }}
                ...
                <div>
                  <div style={{ fontFamily:FSER, fontStyle:'italic', fontWeight:'400', fontSize:'clamp(1.1rem,2.8vw,1.75rem)', color:C.text, letterSpacing:'-0.01em', lineHeight:1.4, marginBottom:'24px' }}>"{q.quote}"</div>
...
var FAQ_ITEMS = [
  ...
  { q:'Do I need to create an account?', a:'No. The core ROI calculator, comparison tool, and city demand heatmap are all free and require no signup. AI-powered features use free credits, after which a lightweight account is optional.' },
  ...
  { q:'Is this only useful for India?', a:'The salary benchmarks and demand data are India-specific — 8 major cities including Bangalore, Hyderabad, Pune, Mumbai, and Delhi. The certification ROI framework itself applies anywhere, but the numbers are calibrated for the Indian job market.' },
      ...
      <AnimatePresence initial={false}>
        {open && (
          <motion.div key="ans" initial={{ height:0, opacity:0 }} animate={{ height:'auto', opacity:1 }} exit={{ height:0, opacity:0 }} transition={{ duration:0.26, ease:[0.16,1,0.3,1] }} style={{ overflow:'hidden' }}>
            <div style={{ paddingBottom:'24px', fontFamily:FB, fontSize:'15px', color:C.text2, lineHeight:'1.8', maxWidth:'68ch' }}>{item.a}</div>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
          ...
          {[35,60,85,110,135].map(function(r,i) {
            return (
              <ellipse key={i} cx="0" cy="0" rx={r} ry={r*0.64}
                ...
                transform="rotate(18)" opacity={i===0?0.6:0.38}
              />
            )
          })}
        </svg>
...
function LandingPage({ onEnter }) {
  ...
  var heroY  = useTransform(heroSP, [0,1], [0, isMobile?20:40])
  ...
  return (
    <div style={{ minHeight:'100vh', background:C.bg }}>
      ...
      {/* ─────────────────────────────────────────────────
          HERO
          - Centered, minimal
          - Compass: top-right corner only, small + quiet
          - Abstract mountain contour: bottom-left, ultra-subtle
          - No subtext under headline
          - One CTA centered
      ───────────────────────────────────────────────── */}
      <div
        ref={heroRef}
      ...
      >
        ...
        {/* COMPASS — top-right corner, small, secondary */}
        <div style={{
          position:'absolute',
        ...
        {/* ABSTRACT MOUNTAIN CONTOUR — bottom-left, ultra-subtle */}
        {/* Thin angular lines suggesting elevation, not a drawing */}
        <div style={{
          position:'absolute',
          bottom: 0, left: 0,
          pointerEvents:'none', zIndex:0,
          ...
          <svg width={isMobile?200:320} height={isMobile?120:190}
            viewBox="0 0 320 190" preserveAspectRatio="xMinYMax meet"
            fill="none" style={{ display:'block' }}>
            {/* Three layered ridge lines — architectural, not scenic */}
            {/* Far ridge */}
            <path
              d="M 0 190 L 0 155 L 40 138 L 80 148 L 120 128 L 160 142 L 200 115 L 240 130 L 280 108 L 320 120 L 320 190 Z"
              fill="rgba(26,25,22,0.04)" />
            {/* Mid ridge */}
            <path
              d="M 0 190 L 0 168 L 30 158 L 70 165 L 110 148 L 150 158 L 190 138 L 230 152 L 270 135 L 320 148 L 320 190 Z"
              fill="rgba(26,25,22,0.055)" />
            {/* Near ridge line only — no fill */}
            <path
              d="M 0 182 L 28 172 L 62 178 L 96 164 L 132 172 L 168 158 L 204 168 L 240 155 L 278 164 L 320 158"
              stroke="rgba(26,25,22,0.09)" strokeWidth="0.8" />
            {/* Elevation tick marks — minimal abstract cue */}
            {[80,160,240].map(function(x, i) {
              return (
                <line key={i}
                  x1={x} y1={190} x2={x} y2={190 - (i+1)*8}
                  stroke="rgba(26,25,22,0.06)" strokeWidth="0.6" />
              )
            })}
          </svg>
        </div>

        {/* Dot grid — very faint, only in hero */}
        <div style={{
          position:'absolute', inset:0,
          backgroundImage:'radial-gradient(rgba(26,25,22,0.048) 1px, transparent 1px)',
        ...
        >
          <div style={{ maxWidth:'960px', margin:'0 auto', padding:isMobile?'0 24px':'0 80px', display:'flex', flexDirection:'column', alignItems:'center' }}>
            ...
            <motion.div
              ...
              style={{ display:'inline-flex', alignItems:'center', gap:'10px', fontFamily:FM, fontSize:'11px', color:C.text3, letterSpacing:'0.12em', textTransform:'uppercase', marginBottom:isMobile?'32px':'44px' }}
            >
            ...
            {/* ── HERO HEADLINE — maximum punch ──
                Structural decisions:
                1. First line: regular weight — sets context
                2. "goldmine" — EB Garamond italic, warm gold — desirable outcome
                3. "mistake" — EB Garamond italic, dark red — the risk
                4. Scale contrast: "either a" and "or a" are smaller,
                   "goldmine" and "mistake" get visual weight
                5. Tight line-height, wide letter-spacing cut
                6. The two outcome words break to their own visual line
                   so the eye stops on each one
            ────────────────────────────────────── */}
            <motion.h1
              ...
              style={{
                fontFamily: FH,
                fontWeight: '700',
                fontSize: isMobile ? 'clamp(2.2rem,9vw,3.2rem)' : 'clamp(3rem,5.5vw,4.8rem)',
                lineHeight: 1.06,
            ...
            >
              ...
              {/* Line 2 — pivot phrase + goldmine */}
              <span style={{ fontWeight:'400', color:C.text2, fontSize:'0.82em', letterSpacing:'-0.015em' }}>
                is either a{' '}
              </span>
              <span style={{
                fontFamily: FSER,
            ...
            {/* CTA — no subtext, centered, anchored */}
            <motion.div
      ...
      {/* Thin section separator — signals transition */}
      <Rule />

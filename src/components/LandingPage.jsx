...
// ─────────────────────────────────────────────────────────
// DESIGN TOKENS — dark editorial, single theme
// ─────────────────────────────────────────────────────────
...
function PrimaryBtn({ onClick, children, large }) {
  return (
    <motion.button
      ...
      onMouseEnter={function (e) { e.currentTarget.style.boxShadow = '0 4px 18px rgba(235,232,227,0.12)' }}
      onMouseLeave={function (e) { e.currentTarget.style.boxShadow = '0 1px 3px rgba(0,0,0,0.4)' }}
    >
      {children}
    </motion.button>
  )
  ...
  return (
    <button onClick={onClick}
      style={{
        ...
        padding: '0 20px', height: '44px', borderRadius: '6px',
        border: '1px solid ' + C.borderMid, background: 'transparent',
        color: C.text2, fontSize: '13px', fontFamily: F_SANS,
        fontWeight: '400', cursor: 'pointer', transition: 'all 0.16s',
      }}
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ width: '28px', height: '1px', background: C.lineHeavy }} />
        <span style={{
          fontFamily: F_MONO, fontSize: '9px', color: C.text4,
        ...
        <motion.span
          ...
          style={{ width: '5px', height: '5px', borderRadius: '50%', background: C.greenVivid, flexShrink: 0 }}
        />
      </div>

      {/* Scrolling rail — each item is a clean pill-like unit */}
      <div style={{ padding: '12px 0 16px', overflow: 'hidden' }}>
        <motion.div
          animate={{ x: ['0%', '-50%'] }}
          ...
          {[...items, ...items].map(function (item, i) {
            return (
              <div key={i} style={{
                ...
                {/* Tag — small green mono label */}
                <span style={{
                  fontFamily: F_MONO, fontSize: '8px',
                  color: C.greenVivid, letterSpacing: '0.1em',
...
// The sticky must be inside a tall container. The cert
// assembles as scroll progresses through that container.
// ─────────────────────────────────────────────────────────
function CertAssembly() {
  ...
  useEffect(function () {
    ...
    // Use scrollY listener for reliable updates
  ...
  if (isMobile) {
    ...
    l2 = 'translateY(' + remap(p8, 0, 1, 50, 0) + 'px) rotateZ(' + remap(p8, 0, 1, -2.5, 0) + 'deg)'
  } else {
  ...
  return (
    // CRITICAL: this outer div must be tall — it's the scroll track
    // position:relative on outer, sticky on inner child
    <div
      ref={trackRef}
      style={{
    ...
    >
      {/* Sticky container — fills the viewport as you scroll through */}
      <div style={{
        position: 'sticky',
        top: 0,
        height: '100vh',
        ...
        {/* Page-color overlay fades in/out to isolate the card */}
        <div style={{
        ...
        <div style={{ position: 'relative', zIndex: 4 }}>
          ...
          <div style={{ transform: 'scale(' + certScale + ')', opacity: certOpacity }}>
            <div style={{
              ...
              {/* LAYER 1 — card frame, animates in */}
              <div style={{ position: 'absolute', inset: 0, transform: l1 }}>
                <svg viewBox="0 0 480 340" width="100%" height="100%"
                  ...
                  <rect x="1.5" y="1.5" width="477" height="337" rx="2.5"
                    fill="none" stroke="url(#cbGrad)" strokeWidth="1.2" />
              ...
              {/* LAYER 2 — card content, animates in from opposite side */}
              <div style={{
          ...
          <div style={{
            opacity: hintOp, marginTop: '40px', textAlign: 'center',
        ...
        {/* Assembled confirmation — appears when done */}
        <div style={{
          opacity: assembledOp,
...
function HowItWorks({ isMobile, onEnter }) {
  var steps = [
    { id: 'basecamp', label: 'Basecamp', subtitle: 'Where you start',  desc: 'Enter your salary, role, and city. Upload your resume for AI to read your actual background. This is your starting elevation.', detail: 'No account required.' },
    { id: 'route',    label: 'Route',    subtitle: 'Choose your path',  desc: 'Select a certification or let the AI recommend the highest-ROI route. Compare up to three routes side by side.',                detail: 'India market data · 2026' },
    { id: 'summit',   label: 'Summit',   subtitle: 'Know the outcome',  desc: 'Exact payback month, 5-year net gain, monthly delta, and a plain statement on whether the climb is worth making.',             detail: 'Your numbers. Your decision.' },
  ]
    ...
    <div style={{ background: C.surface, borderTop: '1px solid ' + C.border, borderBottom: '1px solid ' + C.border, paddingTop: 'clamp(72px,10vw,120px)', paddingBottom: 'clamp(72px,10vw,120px)' }}>
      <div style={{ maxWidth: '980px', margin: '0 auto', padding: isMobile ? '0 24px' : '0 80px' }}>
        ...
        <motion.div variants={RISE} initial="hidden" whileInView="show" viewport={{ once: true }} transition={{ delay: 0.35 }}
          ...
          <PrimaryBtn onClick={onEnter}>Calculate ROI <ArrowRight size={14} /></PrimaryBtn>
...
function ElevenPM({ isMobile, onEnter }) {
  var stories = [
    ...
    { time: '12:03 AM', name: 'Arjun',  loc: 'Pune',      role: 'CS · Fresh graduate',       thought: '"Which cert gets me placed in India?"',    context: 'Three articles recommend AWS. All written by Americans. All in USD.', answer: 'Student Mode. India-specific. GCP placed 47 Pune freshers in Q1 2026.', color: C.text3 },
  ]
          ...
          {quotes.map(function (q, i) {
            ...
            return (
              <motion.div key={i} variants={SLIDE_L} initial="hidden" whileInView="show" viewport={{ once: true }}
                ...
                <div>
                  <div style={{ fontFamily: F_SERIF, fontStyle: 'italic', fontWeight: '400', fontSize: 'clamp(1.1rem,2.8vw,1.75rem)', color: C.text, letterSpacing: '-0.01em', lineHeight: 1.35, marginBottom: '24px' }}>"{q.quote}"</div>
...
var FAQ_ITEMS = [
  ...
  { q: 'What if my certification isn\'t listed?',      a: 'Use the manual calculator — enter cert cost, current salary, and expected hike. It returns payback period and 5-year net gain regardless of whether the cert is indexed.' },
]
...
function FinalCTA({ isMobile, onEnter }) {
  return (
    <div style={{ background: C.surface, borderTop: '1px solid ' + C.border, paddingTop: isMobile ? '80px' : '120px', paddingBottom: isMobile ? '80px' : '120px', position: 'relative', overflow: 'hidden' }}>
      <div style={{ position: 'absolute', right: '-30px', top: '-30px', pointerEvents: 'none', opacity: 0.28 }}>
        <svg width="220" height="220" viewBox="-110 -110 220 220">
          {[28, 52, 76, 100, 124].map(function (r, i) {
            return (<ellipse key={i} cx="0" cy="0" rx={r} ry={r * 0.62} fill="none" stroke={C.lineHeavy} strokeWidth="0.65" transform="rotate(18)" opacity={i === 0 ? 0.55 : 0.32} />)
          })}
        </svg>
      </div>
      <div style={{ maxWidth: '980px', margin: '0 auto', padding: isMobile ? '0 24px' : '0 80px', position: 'relative', zIndex: 1 }}>
        ...
        <motion.div variants={RISE} initial="hidden" whileInView="show" viewport={{ once: true }} transition={{ delay: 0.18 }}
          ...
          <div style={{ fontFamily: F_MONO, fontSize: '11px', color: C.text3, letterSpacing: '0.08em' }}>
    ...
    <div style={{ borderTop: '1px solid ' + C.border, background: C.bg, padding: isMobile ? '32px 24px' : '40px 80px' }}>
      <div style={{ maxWidth: '980px', margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
          ...
          <span style={{ fontFamily: F_SERIF, fontStyle: 'italic', fontWeight: '400', fontSize: '17px', color: C.gold, marginLeft: '1px' }}>ROI</span>
        </div>
...
function LandingPage({ onEnter }) {
  ...
  var imgY      = useTransform(heroSP, [0, 1], [0, isMobile ? 40 : 80])   // image parallaxes slower
  var textY     = useTransform(heroSP, [0, 1], [0, isMobile ? -14 : -32]) // text rises slightly
  var heroOp    = useTransform(heroSP, [0, 0.65], [1, 0])

  return (
    <div style={{ minHeight: '100vh', background: C.bg, overflowX: 'hidden' }}>

      {/* ──────────────────────────────────────────────────
          HERO — real mountain photograph
          Image is position:absolute, bottom-anchored,
          fills right portion, never bleeds past border.
          Text is left column, isolated from mountain layer.
          Vertical lines are editorial coordinates that
          narrate the journey: base → route → summit.
      ─────────────────────────────────────────────────── */}
      <div
        ref={heroRef}
        style={{
          ...
          overflow: 'hidden',  // CRITICAL: clips mountain to this box
          borderBottom: '1px solid ' + C.border,
        }}
      >

        {/* ── MOUNTAIN PHOTOGRAPH — parallax, bottom-anchored ── */}
        <motion.div
          style={{
            y: imgY,
            position: 'absolute',
            // On desktop: right side, 62% wide, reaching peak in upper area
            // On mobile: full width, bottom half
            right: isMobile ? '-10%' : '-4%',
            bottom: '-6%',
            width: isMobile ? '130%' : '65%',
            height: isMobile ? '65%' : '108%',
            zIndex: 1,
            pointerEvents: 'none',
          }}
        >
          {/* The actual mountain photograph */}
          <img
            src="/mountain.png"
            alt="Mountain"
            style={{
              ...
              display: 'block',
              // Fade bottom edge into page bg so it blends cleanly
          ...
          {/* ── ROUTE OVERLAID ON IMAGE ──
              Dashed ascent line + waypoints rendered directly
              over the photograph as an SVG positioned relative
              to the image container.
              Positions are percentages so they scale with the image.
          ── */}
          <svg
            style={{
              position: 'absolute',
          ...
          >
            {/* Route line: from basecamp (bottom-left of mountain) to summit (top-center) */}
            <path
              d="M 28 95 C 30 80, 34 65, 36 52 C 38 40, 38 30, 40 20 C 41 14, 43 9, 46 6"
              fill="none"
              stroke={C.greenVivid}
              strokeWidth="0.5"
            ...
            {/* BASECAMP dot */}
            <circle cx="28" cy="95" r="1.2" fill={C.greenVivid} fillOpacity="0.8" vectorEffect="non-scaling-stroke" />
            <circle cx="28" cy="95" r="2.2" fill="none" stroke={C.greenVivid} strokeWidth="0.4" strokeOpacity="0.4" vectorEffect="non-scaling-stroke" />

            {/* MID-ROUTE dot — month 3 */}
            <circle cx="36" cy="52" r="1.0" fill={C.gold} fillOpacity="0.8" vectorEffect="non-scaling-stroke" />
            <circle cx="36" cy="52" r="1.9" fill="none" stroke={C.gold} strokeWidth="0.35" strokeOpacity="0.4" vectorEffect="non-scaling-stroke" />

            {/* SUMMIT dot */}
            <circle cx="46" cy="6" r="1.4" fill={C.gold} fillOpacity="0.9" vectorEffect="non-scaling-stroke" />
            <circle cx="46" cy="6" r="2.5" fill="none" stroke={C.gold} strokeWidth="0.45" strokeOpacity="0.5" vectorEffect="non-scaling-stroke" />

            {/* Summit flag — small SVG-within-SVG */}
            <line x1="46" y1="6" x2="46" y2="2.5" stroke={C.gold} strokeWidth="0.45" strokeOpacity="0.85" vectorEffect="non-scaling-stroke" />
            <polygon points="46,2.5 49.5,3.8 46,5" fill={C.gold} fillOpacity="0.85" />
          </svg>

          {/* ── INLINE LABELS on the mountain image ── */}
          {/* These float as HTML positioned relative to the image container */}
          {/* BASECAMP label */}
          <div style={{
            position: 'absolute',
            left: '30%',
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
              <div style={{ width: '16px', height: '1px', background: C.greenVivid, opacity: 0.6 }} />
              <div>
                <div style={{ fontFamily: F_MONO, fontSize: '8px', color: C.greenVivid, letterSpacing: '0.12em', textTransform: 'uppercase', opacity: 0.7 }}>Base</div>
              </div>
            </div>
          </div>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
              <div>
                <div style={{ fontFamily: F_MONO, fontSize: '8px', color: C.gold, letterSpacing: '0.12em', textTransform: 'uppercase', opacity: 0.85 }}>Summit</div>
                <div style={{ fontFamily: F_MONO, fontSize: '9px', color: C.gold, letterSpacing: '-0.02em', fontWeight: '500', opacity: 0.9 }}>₹14.2L</div>
              </div>
            </div>
          </div>
        </motion.div>

        {/* Dark gradient — left side receives text, right fades to mountain */}
        <div style={{
          position: 'absolute', inset: 0, zIndex: 2, pointerEvents: 'none',
          background: isMobile
            ? 'linear-gradient(to top, rgba(13,12,11,0.92) 0%, rgba(13,12,11,0.65) 45%, transparent 75%)'
            : 'linear-gradient(to right, rgba(13,12,11,0.98) 0%, rgba(13,12,11,0.92) 32%, rgba(13,12,11,0.55) 55%, transparent 75%)',
        }} />

        {/* ── EDITORIAL VERTICAL LINES ──
            These are not decorative — they narrate the story.
            They represent the elevation chart / ascent metaphor.
            Each line is annotated with a coordinate that tells
            the user where they are in the journey.
            Left line = you are here (base)
            Right line = your destination (summit)
        ── */}
        {!isMobile && (
          <div style={{
            position: 'absolute',
            ...
            {/* Line 1 — LEFT — "your starting point" */}
            <div style={{ position: 'relative', width: '1px', height: '100%' }}>
              {/* The vertical line itself — animated in */}
              <motion.div
                initial={{ scaleY: 0 }}
                animate={{ scaleY: 1 }}
                ...
                style={{
                  ...
                  background: 'linear-gradient(to bottom, transparent 0%, ' + C.border + ' 15%, ' + C.borderMid + ' 50%, ' + C.border + ' 85%, transparent 100%)',
                  transformOrigin: 'top',
                }}
              />
              {/* Top label */}
              ...
              >
                <div style={{ fontFamily: F_MONO, fontSize: '8px', color: C.text4, letterSpacing: '0.14em', textTransform: 'uppercase', whiteSpace: 'nowrap' }}>
                  Base Camp
                </div>
                <div style={{ fontFamily: F_SERIF, fontStyle: 'italic', fontSize: '11px', color: C.text3, whiteSpace: 'nowrap' }}>
                  your salary today
                </div>
              </motion.div>
              {/* Mid marker — month 6 */}
              ...
              >
                <div style={{ width: '4px', height: '1px', background: C.border, position: 'absolute', left: '-8px' }} />
                <div style={{ fontFamily: F_MONO, fontSize: '8px', color: C.text3, letterSpacing: '0.1em', whiteSpace: 'nowrap' }}>
                  Month 6 · break-even
                </div>
              </motion.div>
              {/* Bottom label */}
        ...
        >
          <div style={{
            padding: isMobile ? '0 24px' : '0 0 0 120px',
            maxWidth: isMobile ? '100%' : '50%',
          }}>

            {/* Eyebrow — clean, just text, no extra widgets */}
            <motion.div
            ...
            >
              <div style={{ width: '20px', height: '1px', background: C.borderMid, flexShrink: 0 }} />
              ROI Analysis Platform
            </motion.div>

            {/* ── HEADLINE — exact required text ──
                Structure:
                "Your next cert" — setup line, normal weight
                "is either a goldmine" — pivot, goldmine in warm gold
                "or a mistake." — consequence, muted italic
            ── */}
            <motion.h1
              ...
              style={{
                fontFamily: F_SERIF,
                fontWeight: '400',
                fontSize: isMobile ? 'clamp(2.6rem,9vw,3.8rem)' : 'clamp(2.8rem,5vw,5rem)',
                lineHeight: 0.96,
            ...
            >
              ...
              <br />
              <span style={{ color: C.text3, fontStyle: 'italic' }}>or a mistake.</span>
            </motion.h1>

            {/* ── CTA cluster — clean, no gimmicks ── */}
            <motion.div
            ...
            >
              ...
              {/* Supporting info — one clean line, no pill, no box */}
              <div style={{
                fontFamily: F_MONO,
                fontSize: '11px',
                color: C.text3,
                ...
                <span>Free</span>
                <span style={{ width: '2px', height: '2px', borderRadius: '50%', background: C.text4, flexShrink: 0 }} />
                <span>No account needed</span>
                <span style={{ width: '2px', height: '2px', borderRadius: '50%', background: C.text4, flexShrink: 0 }} />
                <span>India salary data</span>
              </div>
            </motion.div>
      ...
      {/* All sections below — clean sequence */}
      <TrustStrip />
      <CertAssembly />

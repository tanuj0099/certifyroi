...
const MagneticCTA = ({ onClick }) => {
  ...
  const handleClick = useCallback(function (e) {
    ...
    var rect = btnRef.current.getBoundingClientRect()
    var id   = Date.now()
    setRipples(function (prev) { return [...prev, { id, x: e.clientX - rect.left, y: e.clientY - rect.top }] })
    setTimeout(function () {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginTop: '52px', position: 'relative', zIndex: 10 }}>
      ...
      >
        ...
        <motion.span
          ...
          transition={{ duration: 0.45, repeat: hovered ? Infinity : 0 }}
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        ...
        <div ref={heroRef} style={{ maxWidth: '960px', margin: '0 auto', padding: 'calc(var(--nav-h) + 56px) 24px 80px', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
          ...
          <motion.div style={{ y: heroY, opacity: heroOp, scale: heroSc, position: 'relative', zIndex: 1 }}>
            ...
            {/* ── SCROLL HINT — replaces the two removed buttons ── */}
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
            ...
            >
              ...
              <span style={{ fontSize: '10px', fontFamily: F_MONO, color: 'rgba(255,255,255,0.18)', letterSpacing: '0.14em' }}>
                SCROLL
              </span>
            </motion.div>
        ...
        {/* ── ACTUAL NUMBERS ───────────────────────────── */}
        <div style={{ maxWidth: '860px', margin: '0 auto 60px', padding: '0 24px' }}>
          ...
          {/* ── THE ONE MAGNETIC CTA ── */}
          <MagneticCTA onClick={onEnter} />
        </div>

        {/* ── THREE TOOLS BENTO — unchanged ────────────── */}

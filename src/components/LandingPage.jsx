...
// ── CAPSULE BUTTON (no glow, metallic tint) ──
  ...
  return (
    <motion.button
      ...
      style={{
        ...
        background: d
          ? `linear-gradient(135deg, rgba(255,255,255,${h ? 0.07 : 0.045}), rgba(255,255,255,${h ? 0.025 : 0.012}))`
...
// ─────────────────────────────────────────────────────────
// STORY SECTION (with vertical sidebar)
// ─────────────────────────────────────────────────────────
      ...
      <div style={{ maxWidth: '1400px', margin: '0 auto', display: 'flex', flexDirection: isMobile ? 'column' : 'row' }}>
        ...
        {!isMobile && (
          <div style={{ width: '140px', flexShrink: 0, borderRight: `1px solid ${C.border}`, position: 'relative' }}>
            <div style={{
              position: 'sticky', top: '120px', padding: '32px 0',
...
function TrustStrip() {
  ...
  return (
    <div style={{ borderBottom: `1px solid ${C.border}`, background: C.bgAlt, position: 'relative', zIndex: 10, height: '48px', display: 'flex', alignItems: 'center', overflow: 'hidden' }}>
      <div style={{
        position: 'absolute', left: 0, top: 0, bottom: 0, background: C.bgAlt, zIndex: 11,
        borderRight: `1px solid ${C.border}`, display: 'flex', alignItems: 'center',
        padding: '0 20px', boxShadow: `20px 0 20px -10px ${C.bgAlt}`,
      }}>
...
function CertAssembly() {
  ...
  // Desktop: full 3D
  // Mobile: simple scale only (avoids 3D rendering issues)
  let l1, l2
  if (isMobile) {
    l1 = `translateY(${rm(p8, 0, 1, -30, 0)}px) scale(${rm(p8, 0, 1, 0.92, 1)})`
    l2 = `translateY(${rm(p8, 0, 1, 30, 0)}px) scale(${rm(p8, 0, 1, 0.92, 1)})`
  } else {
    l1 = `perspective(1200px) translateZ(${rm(p8, 0, 1, -240, 0)}px) translateY(${rm(p8, 0, 1, -60, 0)}px) rotateY(${rm(p8, 0, 1, 28, 0)}deg) rotateX(${rm(p8, 0, 1, 12, 0)}deg)`
    l2 = `perspective(1200px) translateZ(${rm(p8, 0, 1, 240, 0)}px) translateY(${rm(p8, 0, 1, 60, 0)}px) rotateY(${rm(p8, 0, 1, -22, 0)}deg) rotateX(${rm(p8, 0, 1, -9, 0)}deg)`
  }

  const certScale = prog < 0.8 ? rm(prog, 0, 0.8, 0.64, 1.0) : rm(prog, 0.8, 1.0, 1.0, 0.88)
  const certOpacity = prog < 0.05 ? rm(prog, 0, 0.05, 0, 1) : prog > 0.85 ? rm(prog, 0.85, 1.0, 1, 0) : 1
  const hintOp = prog > 0.16 ? 0 : prog > 0.06 ? rm(prog, 0.06, 0.16, 1, 0) : 1
  const assembledOp = rm(prog, 0.78, 0.88, 0, 1)
    ...
    <div ref={trackRef} style={{
      ...
      <div style={{
            ...
            <div style={{
              ...
              <div style={{ position: 'absolute', inset: 0, transform: l1 }}>
                <svg viewBox="0 0 480 340" width="100%" height="100%" style={{ position: 'absolute', inset: 0, display: 'block' }}>
                  ...
                  <rect x="1.5" y="1.5" width="477" height="337" fill="none" stroke="url(#certGrad)" strokeWidth="1.5" />
                  <rect x="12" y="12" width="456" height="316" fill="none" stroke={C.borderMid} strokeWidth="0.8" />
                </svg>
              </div>
              ...
              <div style={{
                ...
                <div style={{ fontFamily: F_MONO, fontSize: '11px', color: C.gold, letterSpacing: '0.18em', marginBottom: '14px', textTransform: 'uppercase' }}>
                  CertifyROI · Route Analysis
                </div>
                <div style={{ fontFamily: F_SERIF, fontWeight: '400', fontSize: isMobile ? '1.6rem' : 'clamp(1.5rem, 3.2vw, 2.4rem)', color: C.text, marginBottom: '6px', textAlign: 'center', lineHeight: 1.1 }}>
                  Route Briefing
                </div>
                <div style={{ fontFamily: F_SANS, fontSize: '13px', color: C.text3, marginBottom: '28px', textAlign: 'center' }}>
        ...
        <div style={{ opacity: assembledOp, position: 'absolute', bottom: '10%', left: 0, right: 0, textAlign: 'center', pointerEvents: 'none', zIndex: 5 }}>
          <div style={{
            fontFamily: F_MONO, fontSize: '11px', color: C.gold, letterSpacing: '0.15em',
            background: C.surface, display: 'inline-block', padding: '8px 18px',
  ...
  return (
    <StorySection id="02" title="METRICS_LOG" noBorderTop>
      ...
      <motion.div variants={RISE} initial="hidden" whileInView="show" viewport={{ once: true }} style={{ marginBottom: '56px' }}>
        <div style={{
          fontFamily: F_MONO, fontSize: 'clamp(3.5rem, 10vw, 8rem)',
          color: C.text, lineHeight: 1, letterSpacing: '-0.04em', fontWeight: '500',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '14px', marginTop: '14px', flexWrap: 'wrap' }}>
          <div style={{ fontFamily: F_SANS, fontWeight: '500', fontSize: 'clamp(1rem, 2vw, 1.2rem)', color: C.text2 }}>
            5-year net gain · AWS Solutions Architect
          </div>
          <div style={{
            padding: '4px 10px', background: C.surface,
            border: `1px solid ${C.border}`,
            fontFamily: F_MONO, fontSize: '11px', color: C.text3, letterSpacing: '0.08em',
          }}>BLR MEDIAN '26</div>
        </div>
      </motion.div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', borderTop: `1px solid ${C.border}` }}>
        <motion.div variants={RISE} initial="hidden" whileInView="show" viewport={{ once: true }}
          style={{ padding: isMobile ? '36px 0' : '48px 48px 48px 0', borderBottom: isMobile ? `1px solid ${C.border}` : 'none', borderRight: isMobile ? 'none' : `1px solid ${C.border}` }}>
          <div style={{ fontFamily: F_MONO, fontSize: '11px', color: C.text3, letterSpacing: '0.12em', marginBottom: '14px' }}>// PAYBACK_PERIOD</div>
          <div style={{ fontFamily: F_MONO, fontSize: 'clamp(2.5rem, 6vw, 4rem)', color: C.text, lineHeight: 1, letterSpacing: '-0.03em', fontWeight: '500', marginBottom: '14px' }}>
            <CountUp end={6} suffix=" MO" />
          </div>
          <div style={{ fontFamily: F_SANS, fontSize: '14px', color: C.text2, lineHeight: '1.65', maxWidth: '36ch' }}>
        ...
        <motion.div variants={RISE} initial="hidden" whileInView="show" viewport={{ once: true }} transition={{ delay: 0.1 }}
          style={{ padding: isMobile ? '36px 0' : '48px 0 48px 48px' }}>
...
function HowItWorks({ onEnter }) {
  ...
  return (
    <StorySection id="03" title="SYS_ARCHITECTURE" bg={C.surface}>
      <motion.div variants={RISE} initial="hidden" whileInView="show" viewport={{ once: true }}>
        <h2 style={{ fontFamily: F_SANS, fontWeight: '700', fontSize: 'clamp(1.8rem, 4.5vw, 3.2rem)', color: C.text, letterSpacing: '-0.03em', lineHeight: 1.05, margin: '0 0 52px' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0' }}>
        {pairs.map((pair, i) => (
          <motion.div key={i} variants={RISE} initial="hidden" whileInView="show" viewport={{ once: true }} transition={{ delay: i * 0.08 }}
            style={{ paddingTop: i > 0 ? '36px' : '0', paddingBottom: i < pairs.length - 1 ? '36px' : '0', borderBottom: i < pairs.length - 1 ? `1px solid ${C.border}` : 'none' }}>
            <div style={{ fontFamily: F_SANS, fontWeight: '500', fontSize: '16px', color: C.text3, letterSpacing: '-0.01em', marginBottom: '10px', textDecoration: 'line-through', opacity: 0.5 }}>
              {pair.wrong}
            </div>
            <div style={{ display: 'flex', gap: '14px', alignItems: 'flex-start' }}>
              <div style={{ width: '3px', height: '14px', background: C.gold, marginTop: '5px', flexShrink: 0 }} />
              <div style={{ fontFamily: F_SANS, fontSize: '15px', color: C.text, lineHeight: '1.6', fontWeight: '400' }}>{pair.right}</div>
            </div>
          </motion.div>
        ))}
      </div>
        ...
        {stories.map((s, i) => {
          ...
          return (
            <motion.div key={i} variants={RISE} initial="hidden" whileInView="show" viewport={{ once: true }} transition={{ delay: i * 0.08 }}
              onClick={onEnter}
              style={{
                paddingLeft: !isMobile && i > 0 ? '36px' : '0',
                paddingRight: !isMobile && i < 2 ? '36px' : '0',
              ...
              <div style={{ fontFamily: F_SANS, fontSize: '13px', color: C.text2, lineHeight: '1.6', marginBottom: '20px' }}>
        ...
        {quotes.map((q, i) => {
          ...
          return (
            <motion.div key={i} variants={RISE} initial="hidden" whileInView="show" viewport={{ once: true }}
              style={{
                paddingTop: i > 0 ? '40px' : '0', paddingBottom: !last ? '40px' : '0',
                borderBottom: !last ? `1px solid ${C.border}` : 'none',
                display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 160px',
                gap: '20px', alignItems: 'end',
              }}>
              <div>
                <div style={{ fontFamily: F_SERIF, fontStyle: 'italic', fontWeight: '400', fontSize: 'clamp(1.1rem, 2.2vw, 1.6rem)', color: C.text, letterSpacing: '-0.01em', lineHeight: 1.35, marginBottom: '20px' }}>
                  "{q.quote}"
                </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '260px 1fr', gap: isMobile ? '36px' : '64px', alignItems: 'start' }}>
        <div style={{ position: isMobile ? 'static' : 'sticky', top: '140px' }}>
          <motion.h2 variants={RISE} initial="hidden" whileInView="show" viewport={{ once: true }}
            style={{ fontFamily: F_SANS, fontWeight: '700', fontSize: 'clamp(1.6rem, 2.5vw, 2.2rem)', color: C.text, letterSpacing: '-0.025em', lineHeight: 1.15, margin: 0 }}>
...
function FinalCTA({ onEnter }) {
  ...
  return (
    <StorySection id="09" title="SUMMIT_ACCESS" bg={C.surface}>
      <div style={{ position: 'absolute', right: '0', top: '0', pointerEvents: 'none', opacity: 0.2 }}>
        <svg width="360" height="360" viewBox="0 0 360 360">
          {[40, 80, 120, 160, 200].map((r, i) => (
            <circle key={i} cx="360" cy="0" r={r} fill="none" stroke={C.lineHeavy} strokeWidth="1" opacity={1 - (i * 0.15)} />
          ))}
        </svg>
      </div>
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        <motion.h2 variants={RISE} initial="hidden" whileInView="show" viewport={{ once: true }}
          style={{ fontFamily: F_SERIF, fontStyle: 'italic', fontWeight: '400', fontSize: 'clamp(2.8rem, 7.5vw, 5.2rem)', color: C.text, letterSpacing: '-0.02em', lineHeight: 0.96, margin: '0 0 28px' }}>
          You'll know<br />the answer.
        </motion.h2>
        <motion.p variants={RISE} initial="hidden" whileInView="show" viewport={{ once: true }} transition={{ delay: 0.08 }}
          style={{ fontFamily: F_SANS, fontSize: '15px', color: C.text2, lineHeight: '1.65', maxWidth: '42ch', margin: '0 0 40px' }}>
  ...
  return (
    <ThemeContext.Provider value={C}>
      <div style={{
        ...
        background: C.bg,
        color: C.text,
        // 'clip' clips to border-radius WITHOUT breaking position:sticky
        // (unlike 'hidden' which creates a scroll container and breaks sticky)
        overflow: 'clip',
        borderRadius: '14px',
        ...
        <div style={{
          ...
          <div style={{
            ...
            <img src="/mountain.png" alt="" style={{ width: '100%', height: '100%', objectFit: 'contain', objectPosition: 'bottom right' }} />
          </div>
          ...
          <div style={{ position: 'relative', zIndex: 3, width: '100%', maxWidth: '1400px', margin: '0 auto', display: 'flex' }}>
            <div style={{ flex: 1, paddingLeft: isMobile ? '24px' : '8vw', paddingRight: '24px' }}>
              <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}
                style={{ fontFamily: F_MONO, fontSize: '11px', color: C.text3, letterSpacing: '0.18em', textTransform: 'uppercase', marginBottom: '28px', display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              <motion.h1 initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.08 }}
                style={{ fontFamily: F_SERIF, fontWeight: '400', fontSize: isMobile ? 'clamp(2.8rem, 9vw, 5rem)' : 'clamp(4rem, 8vw, 8rem)', lineHeight: 0.95, letterSpacing: '-0.02em', color: C.text, marginBottom: '28px', maxWidth: '14ch' }}>
                Your next cert<br />
                is either a <span style={{ color: C.gold, fontStyle: 'italic', paddingRight: '6px' }}>goldmine</span><br />
                or a <span style={{ color: C.text3, fontStyle: 'italic' }}>mistake.</span>
              </motion.h1>

              <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8, delay: 0.16 }}
                style={{ fontFamily: F_SANS, fontSize: isMobile ? '14px' : '16px', color: C.text2, maxWidth: '400px', lineHeight: '1.6', margin: '0 0 40px' }}>
                Know the exact payback period before you pay the fee. Calculated for your city and current salary.
              </motion.p>

              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8, delay: 0.24 }}
